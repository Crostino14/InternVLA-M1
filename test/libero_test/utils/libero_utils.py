...
def extract_command_from_bddl(bddl_file_path):
    ...
    ADDED from OpenVLA to support L1/L2/L3 command extraction.
...
def get_libero_env(task, model_family, change_command=False, command_level=None, ablation_bddl_file=None, resolution=256):
    """
    Initializes and returns the LIBERO environment, along with the task descriptions.
    UPDATED from OpenVLA to support L1/L2/L3 command variations and ablation study.
...
def save_rollout_video(rollout_traj, idx, success, task_description, log_file=None, dataset_name="libero", run=0, change_command=False, command_level=None):
    """Saves an MP4 replay of an episode."""
    # Build directory path based on configurations
    if change_command and command_level:
        rollout_dir = f"/home/A.CARDAMONE7/outputs/rollouts/libero_goal/syntactic_variation/InternVLA/L3_Finetuned_{command_level}/run_{run}"
    else:
        rollout_dir = f"/home/A.CARDAMONE7/outputs/rollouts/libero_goal/syntactic_variation/InternVLA/L3_Finetuned_50eps/default/run_{run}"
